...
const strColor = ['#ff6b6b', '#ffb347', '#ffd700', '#5ed29c']
...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#070b0a] px-4 py-12 font-['Inter']">

      {/* background glow */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <svg className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px]" viewBox="0 0 700 220" fill="none">
          <defs>
            <filter id="g2"><feGaussianBlur stdDeviation="30"/></filter>
          ...
          <ellipse cx="350" cy="120" rx="300" ry="60" fill="url(#gl2)" filter="url(#g2)" opacity=".35"/>
        </svg>
        {['25%','50%','75%'].map(left => (
      ...
      {/* card */}
      <div className="relative w-full max-w-[420px] rounded-2xl border border-white/[0.08] bg-white/[0.025] backdrop-blur-xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.07)]">
        ...
        <div className="flex items-center justify-between px-7 pt-6">
          ...
          <span className="text-[10px] tracking-[.15em] uppercase text-white/25">New account</span>
        </div>

        {/* title */}
        <div className="px-7 pt-5 pb-1">
          <h1 className="text-2xl font-bold text-white">Create account</h1>
          <p className="text-sm text-white/40 mt-1">Join the game and start drawing</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7 pt-5 pb-7">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {form.password && (
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-[3px] rounded-full bg-white/[.08] overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-300"
                    style={{
                      width: `${(pwStrength / 4) * 100}%`,
                      background: strColor[pwStrength - 1] || '#ff6b6b',
                    }}
                  />
                </div>
                <span
                  className="text-[10px] font-semibold w-10"
                  style={{ color: strColor[pwStrength - 1] || '#ff6b6b' }}
                >
                  {strLabel[pwStrength - 1] || 'Weak'}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              onChange={handleChange}
              required
              className={`w-full bg-white/[.04] border rounded-[10px] px-3.5 py-[11px] text-sm text-white placeholder:text-white/20 outline-none transition-colors ${
                form.confirmPassword && form.password !== form.confirmPassword
                  ? 'border-[rgba(255,107,107,0.5)] bg-[rgba(255,107,107,0.04)]'
                  : 'border-white/10 focus:border-[rgba(94,210,156,0.45)] focus:bg-[rgba(94,210,156,0.04)]'
              }`}
            />
            {form.confirmPassword && form.password !== form.confirmPassword && (
          ...
          <p className="text-center text-xs text-white/30">
            ...
            <Link to="/login" className="text-[#5ed29c] font-semibold hover:underline">
              Log in
            </Link>
          </p>
